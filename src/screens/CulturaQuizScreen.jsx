...
export default function CulturaQuizScreen() {
  const QUIZ_ID = 3;
    ...
    <LinearGradient colors={["#2469A0", "#0D263A"]} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={{ flex: 1 }}>
      <SafeAreaView edges={['top']}>
        <View className="flex-row items-center justify-between px-4 pt-1 pb-2">
          <TouchableOpacity onPress={() => router.back()} accessibilityRole="button" accessibilityLabel="Regresar" hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }} className="w-10 h-10 rounded-full bg-white items-center justify-center">
            <ChevronLeft size={25} color="#2469A0" strokeWidth={2.5} />
          </TouchableOpacity>
          <Image source={require('../../assets/images/logokira2.png')} className="w-[90px] h-[90px]" resizeMode="contain" />
        </View>
      </SafeAreaView>

      <View className="mt-2 items-center">
        <View className="bg-white rounded-full px-5 py-4 shadow items-center" style={{ elevation: 3 }}>
          <Text className="text-[#2469A0] font-extrabold text-md">Cultura Nicaragüense</Text>
        </View>
      </View>
